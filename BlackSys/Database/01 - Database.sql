...
USE master;
GO
IF EXISTS (SELECT * FROM sysdatabases WHERE name='BlackSysDB')
BEGIN
	ALTER DATABASE BlackSysDB SET SINGLE_USER WITH ROLLBACK IMMEDIATE
END
GO
USE master;
IF EXISTS(SELECT * FROM sysdatabases where name='BlackSysDB')
BEGIN
	DROP DATABASE BlackSysDB	
END
GO
CREATE DATABASE BlackSysDB;
GO
USE [BlackSysDB];
GO
