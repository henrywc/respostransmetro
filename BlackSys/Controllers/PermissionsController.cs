...
namespace BlackSys.Controllers
{
    public class PermissionsController : Controller
    {
        ...
        public JsonResult InsertPermission(List<PermissionsInsertModel> permissions)
        {
            ...
            foreach (PermissionsInsertModel permission in permissions)
            {
                db.Permission.Add(new Permission { MenuID = permission.MenuId, RoleID = permission.RoleId });
            }
            try
            {
                db.SaveChanges();
            ...
            //db.Database.ExecuteSqlCommand("delete sec1002 where id in (select id from sec1002 s left join Permission p on s.Roleid = p.roleid and s.Menuid = p.menuid where p.PermissionID is null)");
        ...
        public ActionResult MenuCreate([Bind(Include = "MenuID,DisplayName,ParentMenuID,OrderNumber,MenuURL,MenuIcon,Visible")] Menu menu)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult MenuEdit([Bind(Include = "MenuID,DisplayName,ParentMenuID,OrderNumber,MenuURL,MenuIcon,Visible")] Menu menu)
        {
            if (ModelState.IsValid)
            {
